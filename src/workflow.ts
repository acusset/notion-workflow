...
export class NotionRecipeBookWorkflow extends WorkflowEntrypoint<Env, NotionRecipeBookWorkflowParams> {
	async run(event: WorkflowEvent<NotionRecipeBookWorkflowParams>, step: WorkflowStep) {
		...
		const recipe = await step.do('Use Claude to fetch resource', async () => {
			...
			const prompt = `Fetch "${event.payload.url.toString()}" and extract the recipe found there. Cut out the links, only keep the text. Find a proper dish name to use as title, without the site name or a trailing "Recipe" suffix. Everything should be converted in grams. Include the URL of the recipe's main photo if one is available or the best suitable image.`;
